...
import { fetchReservation, updateReservationStatus, acceptBooking, getOperatorProperties, fetchOperators, suggestNewRoom, sendSuggestNotification } from '../../../../../../Backend/Api/api';
...
const Reservations = () => {
    ...
    const [moderatorProperties, setModeratorProperties] = useState([]);
    ...
    // Handle actions like View, Accept, and Reject
    ...
    const handleMessageBoxSelect = async (mode) => {
        if (mode === 'suggest') {
            try {
                ...
                setModeratorProperties(response.data);
            } catch (error) {
    ...
    const handleConfirmSuggestion = async () => {
        if (selectedProperty && rejectedReservationID.reservationID) {
            ...
            displayToast('error', 'No Property Selected or Reservation Not Found');
        }
    };
    ...
    const handleConfirmNotification = async () => {
        if (rejectedReservationID.reservationID) {
            try {
                const operatorsID = operators.map((operator) => operator.userID);

                await sendSuggestNotification(rejectedReservationID.reservationID, operatorsID);
            ...
            displayToast('error', 'Reservation Not Found');
        }
    };
                ...
                <div className="custom-message-box-overlay">
                    <div className="suggest-properties custom-message-box">
                        ...
                        <div className="property-list">
                            {moderatorProperties.length > 0 ? (
                                moderatorProperties.map((property) => (
                                    <div key={property.propertyID} className="property-option">
                                        <input
                                            type="radio"
                                        ...
                                        />
                                        <label htmlFor={`property-${property.propertyID}`}>
                                            ...
                                            <span className="property-name">{property.propertyName}</span><br />
                                            <span className="property-name">{property.propertyGuestPaxNo}</span><br />
                                            <span className="property-name">RM{property.propertyPrice}</span>
                                        </label>
                                    </div>
                                ))
                            ) : (
                                <p>No property available to suggest</p>
                            )}
                        </div>
                        <button onClick={handleConfirmSuggestion}>Confirm Suggestion</button>
                    </div>
                </div>
            )}
            ...
            {messageBoxMode === 'notify' && (
                <div className="custom-message-box-overlay">
                    <div className="suggest-properties custom-message-box">
                        ...
                        <p>This Will Notify All Operators About This Reservation</p>
